...
void DSB100GeV::Loop()
{
  ...
  TH1F *TempHisto = new TH1F("TempHisto", "TempHisto", 100, 0, 50);
  TH1F *FiberEnergySum = new TH1F("FiberEnergySum", "FiberEnergySum", 100, 5, 30);

  TH1F *Fiber1Energy = new TH1F("Fiber1Energy", "Fiber1Energy", 200, 0, 40);
  TH1F *Fiber2Energy = new TH1F("Fiber2Energy", "Fiber2Energy", 200, 0, 350);
  TH1F *Fiber3Energy = new TH1F("Fiber3Energy", "Fiber3Energy", 200, 0, 300);
  TH1F *Fiber4Energy = new TH1F("Fiber4Energy", "Fiber4Energy", 200, 0, 250);
  ...
  Double_t rms3;
  Double_t rms4;
  Double_t baselineloc1;
  Double_t baseline1;
  Double_t baselineloc2;
  Double_t baseline2;
  Double_t baselineloc3;
  Double_t baseline3;
  Double_t baselineloc4;
  Double_t baseline4;
  Double_t mean;
  Double_t stddev;
  Double_t rms;
  ...
  for (Int_t s=0;s<100;s++) {
    ...
    if (Entry1 >= 50) {
      ...
      //if (abs(Fiber1Mean/(Fiber1Prof[s+1]->GetMean()))>5) Fiber1ProfileHisto->SetBinContent(s+1, Fiber1Prof[s+1]->GetMean());
    ...
    if (Entry2 >= 50) {
      ...
      //if (abs(Fiber2Mean/(Fiber1Prof[s+1]->GetMean()))>5) Fiber2ProfileHisto->SetBinContent(s+1, Fiber2Prof[s+1]->GetMean());
    ...
    if (Entry3 >= 50) {
      ...
      //if (abs(Fiber3Mean/(Fiber3Prof[s+1]->GetMean()))>5) Fiber3ProfileHisto->SetBinContent(s+1, Fiber3Prof[s+1]->GetMean());
    ...
    if (Entry4 >= 50) {
      ...
      //if (abs(Fiber4Mean/(Fiber4Prof[s+1]->GetMean()))>5) Fiber4ProfileHisto->SetBinContent(s+1, Fiber4Prof[s+1]->GetMean());
  ...
  Fiber1ProfileHisto->GetXaxis()->SetTitle("Position (mm)");
  Fiber1ProfileHisto->GetYaxis()->SetTitle("Energy Distribution");
  ...
  XProfile->Divide(1,2);
  XProfile->cd(1);
  Fiber1ProfileHisto->Draw();
  Fiber2ProfileHisto->Draw("same");
  ...
  TFile f("DSB100GeVXProfile.root", "recreate");
